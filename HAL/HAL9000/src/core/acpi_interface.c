    ...
    )
{
    ...
    __try
    {
        ...
        buffer.Pointer = NULL;
        acpiStatus = AcpiGetName(Object, ACPI_FULL_PATHNAME, &buffer);
        if (AE_OK == acpiStatus)
        {
            LOG_TRACE_ACPI("Object full name is: %s\n", (char *) buffer.Pointer);
        }
        else
        {
            LOG_TRACE_ACPI("Could not get Object name\n");
        }
        ...
        {
            acpiStatus = _AcpiParsePrtEntry(Object, busNumber, prtEntry);
            if (AE_OK != acpiStatus)
            {
                LOG_FUNC_ERROR("_AcpiParsePrtEntry", acpiStatus);
                __leave;
            }
...
MSVC_WARNING_SUPPRESS(4127)
    while(TRUE)
    {
        ...
        if(!ACPI_SUCCESS(acpiStatus))
        {
            if (AE_NULL_ENTRY == acpiStatus)
            {
                LOG_TRACE_ACPI("Parents do not have _BBN, assuming bus 0\n");
                bFoundInParent = FALSE;
                busValue = 0;
                acpiStatus = AE_OK;
                break;
...
{
    ACPI_STATUS acpiStatus;
    ACPI_HANDLE pciIntLink;
    ACPI_BUFFER resourceBuffer;

    acpiStatus = AE_OK;
    pciIntLink = NULL;
    memzero(&resourceBuffer, sizeof(ACPI_BUFFER));
    ...
    __try
    {
        ...
        LOG_TRACE_ACPI("Source 0x%x\n", *((BYTE*) (PrtEntry->Source)));
        if (*((BYTE*) (PrtEntry->Source)) != 0)
        {
            LOG_TRACE_ACPI("Source string %s\n", PrtEntry->Source);
        }

        if (*((BYTE*) (PrtEntry->Source)))
        {
            LOG_TRACE_ACPI("PRT entry names PCI Interrupt Link, parsing PIL\n");
            acpiStatus = AcpiGetHandle(NULL, PrtEntry->Source, &pciIntLink);
            if (AE_OK != acpiStatus)
            {
                __leave;
            }
            ASSERT(pciIntLink != NULL);
            LOG_TRACE_ACPI("Successfully obtained PIL for PRT entry\n");

            resourceBuffer.Length = ACPI_ALLOCATE_BUFFER;
            resourceBuffer.Pointer = NULL;
            // Instead of setting an interrupt just get the currently assigned interrupt
            // The simplest solution in my opinion
            acpiStatus = AcpiGetCurrentResources(pciIntLink, &resourceBuffer);
            if (AE_OK != acpiStatus)
            {
                __leave;
            }
            ASSERT(resourceBuffer.Pointer != NULL);
    ...
    __finally
    {
        if (NULL != resourceBuffer.Pointer)
        {
            AcpiOsFree(resourceBuffer.Pointer);
            resourceBuffer.Pointer = NULL;
        }
    }
    ...
    )
{
    ...
    walkContext = (PACPI_RESOURCE_WALK_CONTEXT) Context;
    isInterrupt = FALSE;
    ...
    case ACPI_RESOURCE_TYPE_IRQ:
        irq = (ACPI_RESOURCE_IRQ *) &(Resource->Data.Irq);
        isInterrupt = TRUE;
        interruptCount = irq->InterruptCount;
        irqNumber = irq->Interrupts[0];
        break;
    case ACPI_RESOURCE_TYPE_EXTENDED_IRQ:
        extendedIrq = (ACPI_RESOURCE_EXTENDED_IRQ *) &(Resource->Data.ExtendedIrq);
        isInterrupt = TRUE;
        interruptCount = extendedIrq->InterruptCount;
        irqNumber = extendedIrq->Interrupts[0];
        break;
    default:
        break;
    }
    ...
    if (isInterrupt)
    {
        LOG_TRACE_ACPI("Found IRQ Resource with type %d containing %d interrupt(s), first interrupt is 0x%02x\n", 
            resourceType, interruptCount, irqNumber);
    ...
    {
        LOG_TRACE_ACPI("Found other resource with type %d\n", resourceType);
        if (resourceType != ACPI_RESOURCE_TYPE_END_TAG)
        {
            return AE_BAD_DATA;
        }
    }
