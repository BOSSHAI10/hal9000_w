...
// Structură pentru a transmite indexul N și a primi rezultatul
typedef struct _FIB_CONTEXT {
    int N;
    QWORD Result;
} FIB_CONTEXT, *PFIB_CONTEXT;

static STATUS (CDECL MS_ABI _FibRecursivePSO)(IN_OPT PVOID Context)
{
    PFIB_CONTEXT pCtx = (PFIB_CONTEXT)Context;
    if (pCtx == NULL) return STATUS_UNSUCCESSFUL;
    if (pCtx->N <= 1) { pCtx->Result = (QWORD)pCtx->N; return STATUS_SUCCESS; }

    FIB_CONTEXT c1 = { pCtx->N - 1, 0 }, c2 = { pCtx->N - 2, 0 };
    PTHREAD h1 = NULL, h2 = NULL;
    STATUS exitStatus;

    // Creăm firele pentru calculul recursiv
    if (SUCCEEDED(ThreadCreate("Fib-1", ThreadPriorityDefault, _FibRecursivePSO, &c1, &h1)) &&
        SUCCEEDED(ThreadCreate("Fib-2", ThreadPriorityDefault, _FibRecursivePSO, &c2, &h2))) 
    {
        // Părintele așteaptă (Cerința PSO 30%)
        ThreadWaitForTermination(h1, &exitStatus); ThreadCloseHandle(h1);
        ThreadWaitForTermination(h2, &exitStatus); ThreadCloseHandle(h2);
        pCtx->Result = c1.Result + c2.Result;
    }
    return STATUS_SUCCESS;
}

STATUS (CDECL MS_ABI TestThreadFibonacci)(IN_OPT PVOID Context)
{
    (void)Context; 
    int n = 8; 
    FIB_CONTEXT root = { n, 0 };

    LOG("\n======================================\n");
    LOG("--- CALCUL FIBONACCI RECURSIV N=%d ---\n", n);
    LOG("======================================\n");
    
    STATUS status = _FibRecursivePSO(&root);

    if (SUCCEEDED(status)) {
        // Am adăugat caractere vizibile pentru a găsi rezultatul ușor în log
        LOG("\n >>> [REZULTAT FINAL] Fibonacci(%d) este: %u <<<\n\n", n, (DWORD)root.Result);
    }
    
    // În loc de ThreadSleep, facem un mic "spin" pentru a lăsa log-urile să iasă pe serială
...
const THREAD_TEST THREADS_TEST[] =
{
    ...
    { "Fibonacci", TestThreadFibonacci, _ThreadTestPassContext, (PVOID)6, NULL, NULL, ThreadPriorityDefault, FALSE, TRUE, FALSE },
