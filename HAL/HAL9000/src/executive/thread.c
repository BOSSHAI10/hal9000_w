...
// Schimbăm de la 4 la 8 pentru a avea multipli de 8 
...
MSVC_WARNING_SUPPRESS(4152)
    ...
    {
            // --- COD NOU PENTRU PROIECT ---
        PTHREAD pCurrentThread = GetCurrentThread();
        if (pCurrentThread != NULL)
        {
            pCurrentThread->ChildrenCreatedCount++;

            // ---Folosim AtomicIncrement32 ---
            AtomicIncrement32(&pCurrentThread->ActiveChildrenCount);

            LOG("Thread [ID=%d] este al %u-lea fir creat de firul [ID=%d] pe CPU [ID=%u]\n",
                pThread->Id, 
                pCurrentThread->ChildrenCreatedCount, 
                pCurrentThread->Id, 
                pThread->CreationCpuId);
        }
        // ------------------------------
    ...
    )
{
    ...
    pThread->TotalTicksExecuted++; // Adăugăm tick-ul curent la totalul firului

    // --- COD NOU PENTRU SCHEDULER DINAMIC ---
    // Determinăm limita cuantei actuale (4 implicit, 2 după 16 tick-uri totale)
    DWORD currentQuantumLimit = (pThread->TotalTicksExecuted > 16) ? 2 : 4;
    pThread->CurrentThreadQuantum = currentQuantumLimit; // Actualizăm în structură

    if (++pCpu->ThreadData.RunningThreadTicks >= currentQuantumLimit)
    {
...
{
    PLIST_ENTRY pEntry;
    PTHREAD pFoundThread = NULL;
    INTR_STATE oldState;

    LockAcquire(&m_threadSystemData.AllThreadsLock, &oldState);

    for (pEntry = m_threadSystemData.AllThreadsList.Flink;
         pEntry != &m_threadSystemData.AllThreadsList;
         pEntry = pEntry->Flink)
    {
        PTHREAD pTempThread = CONTAINING_RECORD(pEntry, THREAD, AllList);
        if (pTempThread->Id == Tid)
        {
            pFoundThread = pTempThread;
            _ThreadReference(pFoundThread); 
            break;
        }
    }
    ...
    )
{
    ...
    // =========================================================================
    // LOGICA PSO (Requirement Implementation)
    // =========================================================================
    // Aplicăm logica doar pentru firele utilizator (ID >= 128) 
    // pentru a nu interfera cu firele de boot ale procesoarelor (main-01, main-02 etc.)
    if (pThread->Id >= 128)
    {
        PTHREAD pParent = _ThreadReferenceByTid(pThread->ParentId);
        if (pParent != NULL)
        {
            // 1. Decrementăm numărul de copii activi ai părintelui (Cerința ierarhie)
            int remainingChildren = (int)AtomicDecrement32(&pParent->ActiveChildrenCount);

            // 2. Mesaj LOG conform cerințelor: ID, CPU creare vs CPU terminare, Părinte și Copii rămași
            LOG("Thread [ID=%d] creat pe CPU [ID=%u] se termina pe CPU [ID=%u], parintele [ID=%d] mai are %d copii\n",
                pThread->Id, 
                pThread->CreationCpuId, 
                (BYTE)GetCurrentPcpu()->ApicId, 
                pParent->Id, 
                remainingChildren);
        ...
        // 3. Mesaj LOG pentru cuantele de timp (Cerința Scheduler)
        LOG("Firului de executie [ID=%d] i-au fost alocate %u cuante pana la finalizare\n", 
            pThread->Id, pThread->TotalQuantaAllocated);
    }
    // =========================================================================

    // --- CODUL ORIGINAL HAL9000 (Nu modificăm ordinea de mai jos!) ---
    CpuIntrDisable();
    ...
    // Această funcție este cea care dădea ASSERT-ul roșu dacă logica PSO era greșită
    ...
    )
{
    ...
    __try
    {
        ...
        // --- COD NOU PENTRU PROIECT ---
        // Stocăm CPU-ul pe care se "naște" firul
        pThread->CreationCpuId = (BYTE)GetCurrentPcpu()->ApicId;

        // Identificăm părintele pentru ierarhie
        PTHREAD pCurrentThread = GetCurrentThread();
        pThread->ParentId = (pCurrentThread != NULL) ? pCurrentThread->Id : 0;

        // Inițializăm contoarele pentru copii
        pThread->ChildrenCreatedCount = 0;
        pThread->ActiveChildrenCount = 0;

        // Parametrii pentru scheduler-ul Round-Robin
        pThread->CurrentThreadQuantum = 4; // Începe cu 4 ticuri 
        pThread->TotalQuantaAllocated = 0;
        pThread->TotalTicksExecuted = 0; 
        // ------------------------------
    ...
    )
{
    ...
    if (pNextThread != pCurrentThread)
    {
        ...
        // --- COD NOU: Incrementăm numărul de cuante alocate firului care urmează să ruleze ---
        pNextThread->TotalQuantaAllocated++;
        // ----------------------------------------------------------------------------------
