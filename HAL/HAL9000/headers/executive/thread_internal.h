...
typedef struct _THREAD
{
    ...
    // --- NOILE CÂMPURI ADAUGATE PENTRU PROIECT ---

    // Stochează identificatorul CPU-ului pe care a fost creat firul
    BYTE                    CreationCpuId;

    // Identificatorul firului părinte (cel care a apelat ThreadCreate)
    TID                     ParentId;

    // Câte fire de execuție a creat acest fir în total (pentru mesajul "al X-lea fir")
    DWORD                   ChildrenCreatedCount;

    // Câte fire copil sunt încă în viață (pentru mesajul de la terminare)
    volatile long           ActiveChildrenCount;

    // Cuanta actuală de timp a firului (începe cu 4, scade la 2)
    DWORD                   CurrentThreadQuantum;

    // Numărul total de cuante alocate acestui fir până la finalizare
    DWORD                   TotalQuantaAllocated;

    QWORD                   TotalTicksExecuted;
